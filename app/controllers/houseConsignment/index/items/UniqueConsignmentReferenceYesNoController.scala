...
import pages.houseConsignment.index.items.UniqueConsignmentReferenceYesNoPage
  ...
  def onPageLoad(arrivalId: ArrivalId, houseConsignmentIndex: Index, itemIndex: Index, houseConsignmentMode: Mode, itemMode: Mode): Action[AnyContent] =
    actions.requirePhase6(arrivalId) {
      implicit request =>
        ...
        Ok(view(preparedForm, request.userAnswers.mrn, arrivalId, houseConsignmentIndex, itemIndex, houseConsignmentMode))
    }
      ...
      implicit request =>
        form(houseConsignmentIndex, itemIndex)
          ...
          .fold(
            formWithErrors =>
              Future.successful(
                BadRequest(view(formWithErrors, request.userAnswers.mrn, arrivalId, houseConsignmentIndex, itemIndex, houseConsignmentMode))
              ),
            value =>
              for {
                updatedAnswers <- Future
                  .fromTry(request.userAnswers.set(UniqueConsignmentReferenceYesNoPage(houseConsignmentIndex, itemIndex), value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield {
                val navigator = navigatorProvider.apply(houseConsignmentMode)
                Redirect(navigator.nextPage(UniqueConsignmentReferenceYesNoPage(houseConsignmentIndex, itemIndex), itemMode, updatedAnswers))
              }
          )
    }
}
