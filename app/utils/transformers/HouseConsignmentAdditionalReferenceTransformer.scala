...
class HouseConsignmentAdditionalReferenceTransformer @Inject() (
  referenceDataConnector: ReferenceDataConnector
)(implicit ec: ExecutionContext)
    extends PageTransformer {

  private case class TempAdditionalReference(typeValue: AdditionalReferenceType)
  ...
  def transform(additionalReferences: Seq[AdditionalReferenceType02], hcIndex: Index, itemIndex: Index)(implicit
    ...
    Future.sequence(referenceDataLookups).flatMap {
      _.zipWithIndex.foldLeft(Future.successful(userAnswers))({
        case (acc, (TempAdditionalReference(additionalReference), i)) =>
          acc.flatMap {
            userAnswers =>
