        ...
        "must contain a legend for the question" in {
          ...
          if (mode.isEmpty) {
            if (legendIsHeading) {
              ...
              val labelPrefix = if (args.nonEmpty && args.head == 0) s"$prefix.empty.label" else s"$prefix.label"
              assertElementIncludesText(legends.first(), messages(s"$labelPrefix", args*))
