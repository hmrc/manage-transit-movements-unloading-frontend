...
class UnloadingPermissionActionSpec extends SpecBase with BeforeAndAfterEach with AppWithDefaultMockFixtures {
  ...
  "UnloadingPermissionAction" - {
    "must return 200 when an unloading permission is available" in {
      ...
      when(mockService.getUnloadingPermission(any(), any())(any(), any())).thenReturn(Future.successful(message))
    ...
    "must return 303 and redirect to technical difficulties when no unloading permission is available" in {
      ...
      when(mockService.getUnloadingPermission(any(), any())(any(), any())).thenReturn(Future.failed(new Throwable()))
      val unloadingPermissionProvider = (new UnloadingPermissionActionProvider(mockService)(implicitly))(arrivalId, messageId)
