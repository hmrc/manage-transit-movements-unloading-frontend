...
class DoYouHaveAnythingElseToReportYesNoController @Inject() (
  ...
  view: DoYouHaveAnythingElseToReportYesNoView
  ...
  def onPageLoad(arrivalId: ArrivalId, mode: Mode): Action[AnyContent] = actions.getStatus(arrivalId) {
    implicit request =>
      val preparedForm = request.userAnswers.get(DoYouHaveAnythingElseToReportYesNoPage) match {
        case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(DoYouHaveAnythingElseToReportYesNoPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(DoYouHaveAnythingElseToReportYesNoPage, mode, updatedAnswers))
        )
  }
}
