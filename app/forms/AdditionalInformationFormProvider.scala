      ...
      "value" -> text(requiredError)
        .verifying(
          forms.StopOnFirstFail[String](
            regexp(stringFieldRegexComma, "document.additionalInformation.error.invalidCharacters"),
            maxLength(maxAdditionalInfoLength, "document.additionalInformation.error.length")
          )
        )
    )
}
