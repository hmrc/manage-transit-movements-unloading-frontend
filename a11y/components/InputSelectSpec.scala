...
class InputSelectSpec extends A11ySpecBase {
  ...
  "the 'input select' component" must {
    ...
    val countries: SelectableList[Country] = arbitrary[SelectableList[Country]].sample.value
    ...
    def formProvider                       = new CountryFormProvider()

    val form  = formProvider.apply(NormalMode, prefix, SelectableList(countries.values.toSeq))
