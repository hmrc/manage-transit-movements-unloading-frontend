  ...
  "PackageType" - {
    ...
    "must serialise" in {
      forAll(Gen.alphaNumStr, Gen.alphaNumStr) {
        (code, description) =>
          val value = PackageType(code, description)
          ...
          (code, description) =>
            ...
            Json
              .parse(s"""
                   ...
                   |}
                   |""".stripMargin)
              .as[PackageType] mustEqual value
        }
      }
            ...
            (code, description) =>
              ...
              Json
                .parse(s"""
                         ...
                         |}
                         |""".stripMargin)
                .as[PackageType](PackageType.reads(mockFrontendAppConfig)) mustEqual value
          }
        }
    ...
    "must convert to select item" in {
      forAll(arbitrary[PackageType], arbitrary[Boolean]) {
        (value, selected) =>
    ...
    "must order" in {
      val value1 = PackageType("RS", "Serbia")
      val value2 = PackageType("XS", "Serbia")
      val value3 = PackageType("FR", "France")
