...
class LargeUnsealedGoodsRecordDiscrepanciesYesNoController @Inject() (
  ...
  view: LargeUnsealedGoodsRecordDiscrepanciesYesNoView
  ...
  def onPageLoad(arrivalId: ArrivalId, mode: Mode): Action[AnyContent] = actions.requireData(arrivalId) {
    implicit request =>
      val preparedForm = request.userAnswers.get(LargeUnsealedGoodsRecordDiscrepanciesYesNoPage) match {
        case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(LargeUnsealedGoodsRecordDiscrepanciesYesNoPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(LargeUnsealedGoodsRecordDiscrepanciesYesNoPage, mode, updatedAnswers))
        )
  }
}
