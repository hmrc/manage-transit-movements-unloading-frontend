    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(AddNationalityYesNoPage(transportMeansIndex), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(AddNationalityYesNoPage(transportMeansIndex), mode, updatedAnswers))
        )
  }
}
