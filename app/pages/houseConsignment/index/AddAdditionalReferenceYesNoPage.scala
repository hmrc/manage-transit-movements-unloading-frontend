...
case class AddAdditionalReferenceYesNoPage(houseConsignmentIndex: Index) extends QuestionPage[Boolean] {
  ...
  override def cleanup(value: Option[Boolean], userAnswers: UserAnswers): Try[UserAnswers] = value match {
    case Some(false) => userAnswers.remove(AdditionalReferenceListSection(houseConsignmentIndex))
