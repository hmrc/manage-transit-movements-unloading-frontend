...
class ApplyAnotherItemViewSpec extends ListWithActionsViewBehaviours {
  ...
  override def applyView(form: Form[Boolean]): HtmlFormat.Appendable =
    injector
      ...
      .apply(form, arrivalId.value, notMaxedOutViewModel)(fakeRequest, messages, frontendAppConfig)

  override def applyMaxedOutView: HtmlFormat.Appendable =
    injector
      .instanceOf[ApplyAnotherItemView]
      .apply(formProvider(maxedOutViewModel), arrivalId.value, maxedOutViewModel)(fakeRequest, messages, frontendAppConfig)

  def applyNoMoreItemsView: HtmlFormat.Appendable = injector
    .instanceOf[ApplyAnotherItemView]
    .apply(formProvider(noMoreItemsViewModel), arrivalId.value, noMoreItemsViewModel)(fakeRequest, messages, frontendAppConfig)
