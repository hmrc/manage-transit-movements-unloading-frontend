...
trait ViewModelProvider {

  def prefix(mode: Mode): String
  ...
  def requiredError(houseConsignmentIndex: Index, itemIndex: Index, mode: Mode)(implicit messages: Messages): String =
    mode match {
      case NormalMode => messages(s"${prefix(mode)}.error.required")
      case CheckMode  => messages(s"${prefix(mode)}.error.required", houseConsignmentIndex.display, itemIndex.display)
    }
