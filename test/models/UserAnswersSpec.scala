...
class UserAnswersSpec extends SpecBase with AppWithDefaultMockFixtures {
  ...
  "UserAnswers" - {
    ...
    "addMissingSequenceNumbers" - {
      "when no arrays" - {
        "must return existing data" in {
          val data = Json
            .parse("""
              |{
              |  "foo" : "bar"
            ...
            .as[JsObject]
          val userAnswers = emptyUserAnswers.copy(data = data)
          val result      = userAnswers.addMissingSequenceNumbers()
          result.data mustBe data
        }
      }

      "when there are arrays" - {
        "and all data items have a sequence number" - {
          "must return existing data" in {
            val data = Json
              .parse("""
                |{
        ...
        "and some data items don't have a sequence number" - {
          "must return existing data" in {
            val dataBefore = Json
              .parse("""
                  |{
            ...
            val userAnswers = emptyUserAnswers.copy(data = dataBefore)
            val result      = userAnswers.addMissingSequenceNumbers()
            result.data mustBe dataAfter
          }
        }
      }
    }
