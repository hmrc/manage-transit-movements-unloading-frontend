...
class IE043DataRequiredActionSpec extends SpecBase {

  private class Harness1[T1](page: Gettable[T1])(implicit rds: Reads[T1]) extends IE043DataRequiredAction1[T1](page) {
      ...
      "when required data present in user answers" - {
        "must add value to request" in {
          ...
          forAll(arbitrary[String]) {
            str =>
              val ie043Data = Json.obj(("foo", str))
      ...
      "when required data present in user answers" - {
        "must add value to request" in {
          ...
          forAll(arbitrary[String], arbitrary[String]) {
            (str1, str2) =>
              val ie043Data = Json.obj(("bar", str1), ("foo", str2))
