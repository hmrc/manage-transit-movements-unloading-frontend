...
class AddAnotherDocumentControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with Generators {
  ...
  private val maxItemsSize      = frontendAppConfig.maxTransportDocumentsConsignment + frontendAppConfig.maxSupportingDocumentsConsignment
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to type page at next index" in {
          ...
          userAnswersCaptor.getValue.getValue(AddAnotherDocumentPage) mustEqual true
        }
      }
      ...
      "when no submitted" - {
        "must redirect to next page" in {
          ...
          val userAnswersCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswersCaptor.capture())
          userAnswersCaptor.getValue.getValue(AddAnotherDocumentPage) mustEqual false
        }
      }
    }
