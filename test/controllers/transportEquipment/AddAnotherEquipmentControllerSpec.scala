...
class AddAnotherEquipmentControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with Generators {
  ...
  private lazy val addAnotherEquipmentRoute = controllers.transportEquipment.routes.AddAnotherEquipmentController.onPageLoad(arrivalId, mode).url
  ...
  private val listItems         = Seq.fill(Gen.choose(1, frontendAppConfig.maxTransportEquipment - 1).sample.value)(listItem)
  private val maxedOutListItems = Seq.fill(frontendAppConfig.maxTransportEquipment)(listItem)
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to add container identification yes no page at next index" in {
          ...
          redirectLocation(result).value mustEqual controllers.transportEquipment.index.routes.AddContainerIdentificationNumberYesNoController
            .onPageLoad(arrivalId, mode, notMaxedOutViewModel.nextIndex)
            .url

          val userAnswersCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswersCaptor.capture())
          userAnswersCaptor.getValue.getValue(AddAnotherEquipmentPage) mustEqual true
        }
      }
      ...
      "when no submitted" - {
        "must redirect to next page" in {
          ...
          val userAnswersCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswersCaptor.capture())
          userAnswersCaptor.getValue.getValue(AddAnotherEquipmentPage) mustEqual false
        }
      }
    }
