...
class AdditionalReferenceNumberFormProviderSpec extends SpecBase with AppWithDefaultMockFixtures with StringFieldBehaviours {
  ...
  private val prefix      = "additionalReferenceNumber.index"
  private val requiredKey = s"$prefix.error.required"
  private val invalidKey  = s"$prefix.error.characters"
  private val lengthKey   = s"$prefix.error.length"

  val form = new AdditionalReferenceNumberFormProvider()(requiredKey)
