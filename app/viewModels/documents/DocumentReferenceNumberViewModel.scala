...
object DocumentReferenceNumberViewModel {
  ...
  class DocumentReferenceNumberViewModelProvider @Inject() extends ModeViewModelProvider {
    ...
    override val prefix = "document.referenceNumber"

    def apply(mode: Mode)(implicit messages: Messages): DocumentReferenceNumberViewModel =
      new DocumentReferenceNumberViewModel(heading(mode), title(mode), requiredError(mode))
  }
}
