...
class AddCustomsUnionAndStatisticsCodeYesNoController @Inject() (
  ...
  view: AddCustomsUnionAndStatisticsCodeYesNoView
            ...
            value =>
              for {
                updatedAnswers <- Future
                  .fromTry(request.userAnswers.set(AddCustomsUnionAndStatisticsCodeYesNoPage(houseConsignmentIndex, itemIndex), value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(AddCustomsUnionAndStatisticsCodeYesNoPage(houseConsignmentIndex, itemIndex), mode, updatedAnswers))
          )
    }
}
