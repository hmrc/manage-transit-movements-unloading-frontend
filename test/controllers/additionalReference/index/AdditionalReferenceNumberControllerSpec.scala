...
class AdditionalReferenceNumberControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private val isParagraphRequired = false
  ...
  "AdditionalReferenceNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      contentAsString(result) mustEqual
        view(form, arrivalId, mrn, additionalReferenceIndex, mode, viewModel, isParagraphRequired)(request, messages).toString
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(filledForm, arrivalId, mrn, additionalReferenceIndex, mode, viewModel, isParagraphRequired)(request, messages).toString
    }
