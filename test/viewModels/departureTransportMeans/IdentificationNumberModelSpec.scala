...
class IdentificationNumberModelSpec extends SpecBase with ScalaCheckPropertyChecks with Generators {
  ...
  "must create view model" - {
    ...
    "when CheckMode" in {
      ...
      result.title mustBe "What is the identification number for the new departure means of transport?"
      result.heading mustBe "What is the identification number for the new departure means of transport?"
    }
  }
}
