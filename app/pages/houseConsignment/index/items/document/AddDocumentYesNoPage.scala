...
case class AddDocumentYesNoPage(houseConsignmentIndex: Index, itemIndex: Index) extends QuestionPage[Boolean, Boolean] {

  override def path: JsPath = ItemSection(houseConsignmentIndex, itemIndex).path \ toString

  override def toString: String = "addDocumentYesNo"

  override def cleanup(value: Option[Boolean], userAnswers: UserAnswers): Try[UserAnswers] = value match {
    case Some(false) => userAnswers.remove(DocumentsSection(houseConsignmentIndex, itemIndex))
