...
import viewModels.houseConsignment.index.items.document.AddAnotherHouseConsignmentDocumentViewModel.AddAnotherHouseConsignmentDocumentViewModelProvider
...
class AddAnotherDocumentControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with Generators {
  ...
  private val formProvider: AddAnotherFormProvider = new AddAnotherFormProvider()
  ...
  private lazy val addAnotherDocumentRoute =
    routes.AddAnotherDocumentController
  ...
  override def guiceApplicationBuilder(): GuiceApplicationBuilder =
    super
      ...
      .overrides(bind(classOf[AddAnotherHouseConsignmentDocumentViewModelProvider]).toInstance(mockViewModelProvider))
  ...
  private val maxItemsSize      = frontendAppConfig.maxTransportDocumentsHouseConsignment + frontendAppConfig.maxSupportingDocumentsHouseConsignment
  ...
  private val viewModel: AddAnotherHouseConsignmentDocumentViewModel = houseConsignmentAddAnotherDocumentViewModel.arbitrary.sample.value

  private val notMaxedOutViewModel: AddAnotherHouseConsignmentDocumentViewModel = viewModel.copy(listItems = listItems, allowMore = true)
  private val maxedOutViewModel: AddAnotherHouseConsignmentDocumentViewModel    = viewModel.copy(listItems = maxedOutListItems, allowMore = false)
  ...
  "AddAnotherDocumentController" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" - {
      "when max limit not reached" in {
        ...
        setExistingUserAnswers(emptyUserAnswers.setValue(AddAnotherDocumentPage(houseConsignmentIndex, itemIndex), true))
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to type page at next index" in {
          ...
          userAnswersCaptor.getValue.getValue(AddAnotherDocumentPage(houseConsignmentIndex, itemIndex)) mustEqual true
        }
      }
      ...
      "when no submitted" - {
        "must redirect to next page" - {
          "when normal mode" in {
            ...
            val userAnswersCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
            verify(mockSessionRepository).set(userAnswersCaptor.capture())
            userAnswersCaptor.getValue.getValue(AddAnotherDocumentPage(houseConsignmentIndex, itemIndex)) mustEqual false
          }

          "when check mode" in {
            val addAnotherDocumentRoute = routes.AddAnotherDocumentController
    ...
    "must return a Bad Request and errors" - {
      "when invalid data is submitted and max limit not reached" in {
        ...
        val boundForm = form(notMaxedOutViewModel, houseConsignmentIndex, itemIndex).bind(Map("value" -> ""))
